...
- `api/service-request` — the CV-distribution request form posts here instead of inserting from the browser. The package name and price are read from `BLAST_PACKAGES` on the server, so a forged POST cannot invent a package or a price nobody was shown — and those numbers are the only thing the page exists to measure. It also messages `TELEGRAM_ADMIN_CHAT_ID` on each request (best-effort; a failed notification never loses the row) and refuses a sixth request from one address within an hour. `20260902010000_service_requests_api_only.sql` drops the anonymous insert policy so this route is the only writer.
  An attached CV arrives as multipart (the route still accepts plain JSON when there is none) and goes into the **private** `service-cv` bucket, which has no storage policies at all — a CV holds a passport number, visas and a date of birth, the same reason `cv_share_tokens` exists. Only the service role can reach it. The file is also pushed to the operator's Telegram chat with `tgSendDocument`, so no readable URL for it exists anywhere; `api/admin/service-request-cv` mints a ten-minute signed URL per click for the admin screen, taking the object path from the row rather than the query. The upload runs before the insert and its failure is logged, not fatal: losing the request over a storage hiccup would cost more than losing the file.
...
**The CV and the package are both required**, in the form and again in the route — the form is not a boundary. The CV is the work: a request without one is a lead someone has to chase before anything can start. The package is what makes the row mean anything: without it there is no base, no price and no number of sends, so the request can be neither quoted nor counted. The package select keeps an empty first option, disabled, rather than dropping it — an empty select would show the first package as if it had been picked. Note the split in `api/service-request` — the *file* must be present, but its *upload* is allowed to fail, because the contacts are still worth keeping. `/admin/service-requests` puts the counts per package and per page language above the list, because that aggregate *is* the deliverable, and `TELEGRAM_ADMIN_CHAT_ID` gets a message per request so nobody has to poll that screen.
