...
function RequestForm({
  ...
  async function submit(e: React.FormEvent) {
    e.preventDefault();
    // Checked in the order the fields are read, so the message points at the
    // first thing that is missing rather than the last.
    //
    // The package is refused here and again on the server. A request without one
    // says only "somebody wants something": it names no base, no price and no
    // number of sends, so it can be neither quoted nor counted — and the counts
    // per package are the only thing this page exists to produce.
    if (!chosen) {
      setError(copy.errPackage);
      return;
    }
    ...
    <form onSubmit={submit} className="mt-5 flex flex-col gap-3">
      <div className="rounded-xl border border-white/10 bg-navy2 px-4 py-3">
        ...
        {onChoose ? (
          ...
          >
            {/* Disabled, not absent: an empty select would show the first
                package as if it had been picked. */}
            <option value="" disabled>{copy.fPick}</option>
            {GROUP_ORDER.map((group) => {
      ...
      {/* The CV, required: it is the work. A request without one is a lead
          somebody has to chase before anything can start. */}
      <div className="rounded-xl border border-white/10 bg-navy2 px-4 py-3">
        {cv ? (
      ...
      <button
        type="submit"
        disabled={sending || !chosen || !values.email.trim() || !cv}
