...
async function fetchCountryVacancies(keywords: string[]): Promise<Vac[]> {
  ...
  const { data } = await getServerSupabase()
    .from("vacancies")
    .select("id, title, rank, vessel_type, salary_from, salary_to, salary_period, currency, contract_duration, country, joining_date, companies(name, location, is_verified)")
    .eq("is_active", true)
