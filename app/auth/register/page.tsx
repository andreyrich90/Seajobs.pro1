  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      if (role === "seafarer") {
        ...
        router.push(localePath("/seafarer/dashboard", lang));
      } else {
        if (typeof window !== "undefined") localStorage.setItem("user_role", "company");
        router.push(localePath("/company/dashboard", lang));
      }
    } catch {
        ...
        <div className="w-full max-w-lg">
          ...
          {step === 1 && (
            <div>
              <div className="mb-8 text-center">
                <h1 className="font-display text-3xl font-semibold text-white">{t.reg_title}</h1>
                <p className="mt-2 text-sm text-mist">{t.reg_subtitle}</p>
              </div>
              ...
              <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
                ...
                >
                  ...
                  <div className="text-center">
                    <h2 className="font-display text-xl font-semibold text-white">{t.tab_seafarer}</h2>
                    <p className="mt-1.5 text-sm text-mist leading-relaxed">
                      {t.card_seafarer_desc}
                    </p>
                  </div>
                </button>
              ...
              <div className="mt-6">
                <div className="mb-4 flex items-center gap-3">
                  ...
                  <span className="text-xs text-mist">{t.or_signup}</span>
                  <div className="h-px flex-1 bg-white/10" />
                </div>
                <button
          ...
          {step === 2 && emailSent && (
            <div className="rounded-2xl border border-teal/30 bg-teal/10 p-8 text-center">
              ...
              <h1 className="font-display text-2xl font-semibold text-white">{t.confirm_email}</h1>
              <p className="mt-2 text-sm text-foam/80">
                {t.confirm_sent_pre}{" "}
                <span className="font-semibold text-foam">{email}</span>. {t.confirm_sent_post}
              </p>
              <Link
                href="/auth/login"
          ...
          {step === 2 && !emailSent && (
            <div>
              ...
              <div className="rounded-2xl border border-white/10 bg-card p-8">
                ...
                {/* Google first — sign up in one tap, or fill the form below. */}
                <button
                ...
                <form onSubmit={handleSubmit} className="flex flex-col gap-5">
                  ...
                  <button
                    type="submit"
                    disabled={loading}
                    className="mt-2 flex items-center justify-center gap-2 rounded-xl bg-gradient-to-br from-brass to-brass2 px-5 py-3 text-sm font-bold text-[#061523] transition hover:-translate-y-0.5 disabled:opacity-50 disabled:translate-y-0"
                  >
                    {loading ? (
                      <span className="text-[#061523]/70">{t.creating}</span>
                    ) : (
                      t.create_account
                    )}
                  </button>
